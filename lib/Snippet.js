    ...
    remainingTags.forEach(tag => {
      ...
      switch (name) {
        case 'param':
          ...
          child.applyCommentTag(tag, comment)

          (method || this).parameters.set(child.label, child)
          break
        ...
        case 'property':
          ...
          (method || this).properties.set(child.label, child)
          break
    ...
    if (this.start.line < line && this.end.line > line) {
      ...
      for (let i = 0; i < attributes.length; i++) {
        if (this[attributes[i]] instanceof Map || this[attributes[i]] instanceof Set) {
          ...
          while (!shortCircuit && !(iterator = list.next()).done) {
