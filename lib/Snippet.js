...
class Snippet extends EventEmitter {
  ...
  processOrphanComment (comment) {
    ...
    comment.tags = comment.tags.filter(tag => {
      switch (tag.tag) {
        ...
        // case 'property':
        //   property = new DOC.Property(null, this.SOURCE, this)
        //   property.label = tag.name || tag.value
        //   property.description = tag.description
        //
        //   LAST_ENTITY.properties.set(property.label, property)
        //
        //   LAST_ENTITY = property
        //
        //   return false

        // case 'type':
        // case 'typedef':
        //   entity = new DOC.TypeDefinition(null, this.SOURCE, this)
        //   entity.label = tag.name || tag.value
    ...
    comment.tags.forEach(tag => {
      switch (tag.tag.toLowerCase()) {
        ...
        case 'property':
          ...
          if (!child.ignored) {
            ;(method || entity || this).properties.set(child.label, child) // eslint-disable-line
          }

          break

        case 'ignore':
          ;(method || entity || child || this).ignored = true
          break

        default:
          ;(method || entity || child || this).applyCommentTag(tag, comment)
      }
    })

    if (method && !method.ignored) {
      this.methods.set(method.label, method)
    }

    if (entity && !entity.ignored) {
      BUS.emit('register.type', entity)
    }
                ...
                switch (emitMethod) {
                  ...
                  case 'funnel':
                    ...
                    if (node.arguments && node.arguments[1].type.toLowerCase() === 'literal') {
                      ...
                      evt.label = me.getEventName(node.arguments[1])
                      evt.parameters = parameters
                      evt.node = node
                  ...
                  case 'threshold':
                    if (node.arguments.length >= 3 && node.arguments[2].type.toLowerCase() === 'literal') {
                      ...
                      let eventCt = me.getEventCount(node.arguments[1])
                      let eventName = me.getEventName(node.arguments[0])

                      evt.description = `Triggered after ${eventName} is fired ${eventCt} time${parseInt(eventCt, 10) === 1 ? '' : 's'}.`
                      evt.parameters = parameters
                      evt.node = node
  ...
  createStandardProperty (node) {
    ...
    node.value.properties.forEach(arg => {
      switch (arg.key.name.toLowerkCase()) {
        case 'enumerable':
  ...
  getEventName (arg) {
    return !arg.hasOwnProperty('value') ? `\`${arg.name}\` (a Symbol)` : '`' + arg.value + '`'
  }

  getEventCount (arg) {
    return !arg.hasOwnProperty('value') ? `\`${this.getName(arg)}\`` : '`' + arg.value + '`'
  }
}
