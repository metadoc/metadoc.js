    ...
    if (node.type.toLowerCase() === 'assignmentpattern' && node.right) {
      ...
      if (this.datatype === 'undefined') {
        switch (node.right.type.replace('Expression', '').toLowerCase()) {
          ...
          case 'identifier':
            setTimeout(() => this.emit('warning', `Identifier ${node.right.name} at ${this.sourcefile}:${node.right.loc.start.line}:${node.right.loc.start.column} may not be a valid default value.`), 0) // eslint-disable-line no-fallthrough
